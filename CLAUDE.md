...
`lazyLoad` sets the global tech string. When no framework is detected, `lazyLoad` no longer aborts — it falls back to the `generic` crawler (`src/lazyLoad/generic/`) and sets tech to `"generic"`, so `run`'s exit code 10 ("Technology not detected") now only fires if lazyload throws before reaching that fallback. Techs other than `"next"`, `"vue"`, `"nuxt"`, `"react"`, `"svelte"`, and `"angular"` — including `"generic"` — only get lazyload; the rest of the pipeline is skipped with a warning.

**`generic` tech**: used for sites running no supported framework. `generic_crawlPages.ts` recursively BFS-crawls in-scope `<a href>` links starting from `-u` (capped by `--max-pages`), running JS discovery on every visited page and downloading incrementally as it goes — the JS a site's landing page references is rarely all the JS the site has. Unless the caller sets `-s`/`--strict-scope`, the crawl's default scope is the host reached after following redirects from `-u` (`generic_resolveScope.ts`, capped by `--max-redirects`, default 20) rather than the unrestricted `*` default used elsewhere — otherwise an unscoped generic crawl would follow every in-page link across the entire web. On each page, `generic_getScriptTags.ts` seeds from `<script src>`/inline scripts/`<link rel="modulepreload">` (decoding `data:` URI script sources and skipping non-JS `<script type="...">` values like `application/ld+json`/`speculationrules`); `generic_scanAttributesForJs.ts` additionally walks every HTML attribute value, resolves it with the `URL` constructor, and — for any URL with a path segment ending in `.js` (catches cachebuster-suffixed paths like `.../beacon.min.js/v124/token` that don't end in `.js` themselves) — confirms it's actually JavaScript via a GET `Content-Type` check against a small allow-list (`generic_jsMimeTypes.ts`: `text/javascript` per RFC 9239, plus RFC 4329's now-obsoleted `application/javascript`/`application/ecmascript` and other legacy variants) rather than trusting the file extension alone. Downloaded files are written by a dedicated `generic_downloadFiles.ts` writer, not `downloadFilesUtil.ts`/`DownloadQueue`, since those derive the on-disk filename from an end-anchored match against the URL's last path segment — which fails for the cachebuster shape above. Every generic-downloaded file (script-src, inline, data-URI) carries a `// File Source: <url>` header as its first line — this is what lets `generic_stringsDiscovery.ts` (below) resolve strings found inside a file against that file's own URL.

**`--strings` (generic tech only)**: chains the `strings` module (`src/strings/index.ts`) into the generic crawl to catch JS referenced only as a string literal inside an already-downloaded file — e.g. a WordPress plugin's `"pdfWorker": "https://.../pdf.worker.js"` config embedded in an inline `<script>`. `generic_stringsDiscovery.ts` runs `strings()` against the whole output directory (files span multiple host subdirectories — the page's own host plus any third-party asset hosts), recovers each file's source URL from its `// File Source:` header, resolves each extracted string against **that file's own URL** (not a single global base — unlike `next_SubsequentRequests.ts`'s pattern of resolving every extracted path against one fixed target URL), and confirms candidates via the same `confirmJsContentType` helper attribute-scanning uses. New downloads feed the next `strings()` pass; this repeats until a pass finds nothing new or `--strings-max-iterations` (default 5, `0` = unlimited) is reached. Two related gotchas discovered while building this: (1) `resolveJsPathCandidate` must reject any value starting with `#` or `?` before resolving — a fragment/query-only string resolved against a `.js` base URL inherits that base's pathname unchanged, so e.g. a CSS selector string embedded in a bundle (`"#some-selector{...}"`) would otherwise falsely look like a distinct JS candidate and cause runaway duplicate re-downloads of the same file; (2) `strings()`'s per-file parse step must catch and skip individual parse failures rather than aborting the whole extraction run — a single non-JS `<script>` saved with a `.js` extension (guarded against above, but third-party files can still have unusual syntax) would otherwise silently zero out every candidate, not just the bad file's.
