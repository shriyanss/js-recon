...
const mitmproxy = async () => {
  ...
  const mitmdumpArgs = [
    "-s",
    globals.getMitmParseScript(),
    "-p",
    globals.getMitmPort(),
  ];
  console.log(chalk.cyan(`[i] Running mitmdump with the following command:`));
  console.log(chalk.bgCyan(`mitmdump ${mitmdumpArgs.join(" ")}`));
  ...
  mitmdumpProcess.stdout.on("data", (data) => {
    ...
    if (output.includes("HTTP(S) proxy listening at")) {
      console.log(
        chalk.green(
          `[✓] mitmdump started successfully and listening on port ${globals.getMitmPort()}`,
        ),
      );
    }
  });
  ...
  mitmdumpProcess.on("error", (error) => {
    console.error(
      chalk.red(`[!] Failed to start mitmdump process: ${error.message}`),
    );
  });

  return mitmdumpProcess;
};
