  ...
  if (text === "") {
    ...
    if (text.split(" ").length < 2) {
      ...
      if (option === "") {
        ...
        outputBox.log(commandHelpers.navHistory(chunks, state.functionNavHistory));
        state.lastCommandStatus = true;
      } else {
    ...
    if (text.split(" ").length < 2) {
      ...
      if (funcName === "") {
        ...
        printFunction(outputBox, funcCode, chunks[funcId].description, state.funcWriteFile);
        state.lastCommandStatus = true;
    ...
    if (text.split(" ").length < 3) {
      ...
      if (option === "funcwritefile") {
        ...
        outputBox.log(
          chalk.green(`Function write file set to ${state.funcWriteFile}`)
        );
        state.lastCommandStatus = true;
      } else {
        outputBox.log(chalk.red(option), "is not a valid option");
