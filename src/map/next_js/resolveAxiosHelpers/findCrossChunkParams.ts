                ...
                CallExpression(path) {
                    ...
                    const extractAndRecordParams = (callNode, context = '') => {
                        if (callNode.arguments && callNode.arguments.length > 0) {
                            ...
                            const callSnippet = chunkCode.substring(
                                callNode.start, 
                                callNode.end
                            );
                            ...
                            console.log(
                                chalk.cyan(`[+] Found ${context} parameter usage for '${exportName}' in chunk ${importingChunkName} ("${functionFile}":${functionFileLine})`)
                            );
                    ...
                    if (path.parentPath && path.parentPath.isAwaitExpression() && isTargetExportCall(path.node.callee)) {
                    ...
                    const checkForComplexAssignments = () => {
                        ...
                        const funcDecl = path.findParent(p => 
                            ...
                            ((p.node.init && (p.node.init.type === "ArrowFunctionExpression" || p.node.init.type === "FunctionExpression")) ||
                             (p.node.right && (p.node.right.type === "ArrowFunctionExpression" || p.node.right.type === "FunctionExpression")))
                        );
                    ...
                    if (path.parentPath && 
                        ...
                        ((path.parentPath.node.consequent === path.node) || 
                         (path.parentPath.node.alternate === path.node))) {
                                    ...
                                    if (arg.type === "ObjectExpression") {
                                        ...
                                        for (const prop of arg.properties) {
                                            if (prop.type === "ObjectProperty" && 
                                                ...
                                                prop.value.object && prop.value.object.type === "Identifier" &&
                                                prop.value.object.name === "e") {
                                                // This is likely a parameter derived from our export
                                                console.log(
                                                    chalk.cyan(`[+] Found conditional branch parameter usage for '${exportName}' in chunk ${importingChunkName} ("${functionFile}":${functionFileLine})`)
                                                );
                                                console.log(chalk.magenta(`    Params: ${params}`));
