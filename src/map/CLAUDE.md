...
- **Caller-lookup total-size cap (50 MB).** `vue_resolveXhr` and `vue_resolveHttpClient` apply the same 1.5 MB per-file guard _and_ a 50 MB cumulative total when building `allFilePaths` for `makeGetCallers`. Without this, sites that download hundreds of small third-party source files (e.g. a polyfill or compiler source tree pulled in via unrestricted scope) cause `buildAliasMap` to parse all of them at once and exhaust the heap. The per-file scanning loop is unaffected — it still processes every file under 1.5 MB.
