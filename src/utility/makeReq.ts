...
const reportFailure = (url: string, err: unknown): void => {
    ...
    if (globals.getCacheOnly()) {
        console.error(chalk.dim(`[!] Cache miss (cache-only mode): ${url}`));
        return;
    }
    console.error(chalk.red(`[!] Failed to fetch ${url} : ${err}`));
...
const writeCache = async (url: string, headers: {}, response: Response): Promise<void> => {
    try {
        ...
        console.error(chalk.yellow(`[!] Failed to write response cache for ${url}: ${err?.message || err}`));
    }
};
...
const writeCacheUnsafe = async (url: string, headers: {}, response: Response): Promise<void> => {
    ...
    try {
        ...
        if (err instanceof RangeError) {
            ...
            console.error(chalk.yellow(`[!] Cache too big... Emptying cache`));
    ...
    while (true) {
        ...
        try {
            ...
            if (err.name === "AbortError") {
                console.error(chalk.red(`[!] Request to ${url} timed out after ${requestTimeout}ms`));
                return null;
            }
            if (counter > 10) {
...
const handleFirewall = async (url: string, resp_text: string): Promise<string | null> => {
    if (resp_text.includes("/?bm-verify=") || resp_text.includes("<title>Just a moment...</title>")) {
        console.error(chalk.yellow(`[!] CF Firewall detected. Trying to bypass with headless browser`));
        ...
        console.error(chalk.yellow(`[!] Cloudfront Firewall detected. Trying to bypass with headless browser`));
        const browser = await puppeteer.launch({
            headless: true,
            args: globals.getDisableSandbox() ? ["--no-sandbox", "--disable-setuid-sandbox"] : [],
        });
