...
**Implementation:** `src/refactor/remote/version-detect.ts`

- `generateBundleSignatures(chunks, scat, extraCodes?)` — runs `cs_mast_init` on every chunk + optional extra code snippets (e.g. vendor chunks for code-split webpack apps). Uses `sourceType: "unambiguous"` to handle both module and script bundles.
- `fetchReliableSignatures(bundler, version, scatDir)` — downloads `version/react/<bundler>/<version>/<scatDir>/reliable_signatures.json` from the `shriyanss/cs-mast-s-dataset` HF bucket, caches locally with a 7-day TTL.
- `detectReactVersion(chunks, tech, scatDir, extraCodes?)` — lists available versions, generates bundle sigs, counts matches per version, returns the best match.
