...
- `remote/cache.ts` — manages two cache layers: (1) the file list cache (`~/.js-recon/refactor/cs-mast-s-list-cache.json`, refreshed every 7 days or on `--refresh-cache`); (2) per-file signature cache (`~/.js-recon/refactor/signature_cache/<branch>/<subpath>/collisions.json` + `cached_at.txt` + `remote_hash.txt`). Eviction runs whenever a new file is saved and the cache dir exceeds `maxCacheSizeMb` — oldest entries are deleted until the dir is below 50% of the limit.

### Content-based cache validation (upstream change detection)

The per-file signature cache's 7-day age-based TTL alone cannot detect that the upstream HuggingFace dataset content changed (e.g. after a regeneration or fix) within that window — a stale-but-not-yet-expired cache entry would be trusted, silently producing non-deterministic `refactor` results (see `nextjs-webpack-refactor-cache-invalidation` in `js-recon-internal-docs` work item #51). Every non-`--skip-cache-checks` run now also validates cache entries against upstream content:

1. `listCollisionsFileHashes(branch, scatDir)` in `remote/hf-client.ts` fetches the bucket tree API once per branch/scat and returns a `Map<subpath, xetHash>` — the bucket's own content hash per file, independent of any mtime/age. `refactor/index.ts` and `run/bundler-detect.ts` each call this once before their per-file cache-check loop.
2. Each cached signature file's last-known remote hash is persisted alongside it in `remote_hash.txt`. `isSignatureCacheFresh()` now takes an optional `remoteHash` argument: when the current run's tree fetch produced a hash for that subpath, a mismatch against the stored `remote_hash.txt` immediately invalidates the cache **regardless of age** (see `isCacheContentStale()` for the pure comparison — testable independent of the filesystem I/O in `cache.test.ts`). When the tree fetch didn't return a hash for that subpath (network error, or the path wasn't found), behavior falls back to the pre-existing age-based TTL check.
3. `--skip-cache-checks` skips the tree-hash fetch entirely and trusts existing cache files unconditionally (unchanged from before).

**Manual cache purge** (recommended before any benchmark or regression comparison, on top of the above): `rm -rf ~/.js-recon/refactor/{signature_cache,cs-mast-s-list-cache.json,version_sigs_cache,config.json}`.
