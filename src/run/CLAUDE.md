...
- **`--max-heap` re-exec.** `applyHeapLimit()` (`src/utility/heap.ts`) is called at the very start of the `run` action handler in `src/index.ts`, before any pipeline work. If the requested heap differs from the current V8 limit by more than 10%, the entire process re-execs with `--max-old-space-size=<target>`. The env var `JS_RECON_HEAP_SET=1` prevents a second re-exec. Because re-exec happens before `processUrl` is called, all pipeline steps — including the heap-intensive `map` step — run under the correct limit.
