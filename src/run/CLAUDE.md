...
- `interruptHandler.ts` — SIGINT (Ctrl-C) handler. Installs a persistent `process.on('SIGINT')` listener while `run` is active. On interrupt, prints a menu and reads one line from stdin. Exposes `getSkipStepPromise()` (each step in `processUrl` is wrapped in `Promise.race([step, getSkipStepPromise()])` so choosing "skip step" causes the pipeline to advance without waiting for the current step to finish) and `shouldSkipTarget()` (checked between steps; returning early from `processUrl` skips the remaining steps for the current target). In batch mode, `resetSkipTarget()` is called before each target.
    - **Puppeteer's own `handleSIGINT` must be disabled while this handler is active.** Puppeteer's default `puppeteer.launch()` options register their own `SIGINT` listener that kills the Chrome child process immediately on Ctrl-C, independent of this file's handler. That out-of-band kill rejects whatever step is in flight, and since the outer pipeline `try/finally` in `index.ts` unconditionally calls `process.exit()` on its way out, the process would terminate before the menu's `readline` prompt could resolve — printing the menu but never actually waiting for input. Every `puppeteer.launch()` call site in the codebase passes `handleSIGINT: !isSigintHandlerActive()` (exported from this file) so Puppeteer only manages the browser's SIGINT lifecycle when `run`'s own handler is _not_ installed (e.g. standalone `lazyload`). `waitForPendingInterrupt()` is a second line of defense: the outer `finally` in `index.ts` awaits it before calling `process.exit()`, so any pending menu prompt is allowed to resolve first regardless of what triggered the exit path.
