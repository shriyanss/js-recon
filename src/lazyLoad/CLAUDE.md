...
- `generic/` — fallback crawler used when `techDetect/` returns no match. Not a real framework, but it does recursively crawl the site: `generic_crawlPages.ts` BFS-walks in-scope `<a href>` links from `-u` (capped by `--max-pages`), downloading JS incrementally per page. Default scope (unless `-s`/`--strict-scope` is set) is the host reached after following redirects from `-u` (`generic_resolveScope.ts`, capped by `--max-redirects`) — otherwise an unscoped crawl would follow every in-page link across the entire web. Per page: `generic_getScriptTags.ts` seeds from `<script src>`/inline scripts/`<link rel="modulepreload">` (same pattern as `react_getScriptTags.ts`; decodes `data:` URI script sources, skips non-JS `<script type="...">` values like `application/ld+json`/`speculationrules`); `generic_scanAttributesForJs.ts` additionally walks every HTML attribute value, resolves it with the `URL` constructor, and — for any URL with a path segment ending in `.js` (catches cachebuster-suffixed paths like `.../beacon.min.js/v124/token` that don't end in `.js`) — confirms it's actually JavaScript via a GET `Content-Type` check (`generic_jsMimeTypes.ts`) rather than trusting the extension alone. `generic_downloadFiles.ts` is a dedicated writer (not `downloadFilesUtil.ts`/`DownloadQueue`) because those derive the on-disk filename from an end-anchored match against the URL's last path segment, which fails for the same cachebuster shape; every file it writes (plus `generic_getScriptTags.ts`'s inline/data-URI writes) carries a `// File Source: <url>` header as its first line. When `--strings` is set, `generic_stringsDiscovery.ts` chains the `strings` module in after each page-crawl round: runs `strings()` over the whole output directory, recovers each file's source URL from its header, resolves extracted string literals against _that file's own URL_ (not a single global base), confirms via Content-Type, and recurses — new downloads feed the next pass — until nothing new is found or `--strings-max-iterations` is hit. See root `CLAUDE.md`'s "Tech detection flow" section for the two gotchas this surfaced (fragment/query-only strings falsely resolving against a `.js` base's own pathname; a single unparseable file aborting the whole `strings()` run).
- `stagnation/` — `stagnationMonitor.ts` exports `StagnationMonitor`, a small state machine used only by the `generic` crawler to detect when an effectively unbounded site (blog, news feed) has stopped surfacing new JS content. It reads content-hash counts from two mutable globals in `globals.ts` (`recordJsFileHash`/`getJsFileHashCounts`/`getJsFileTotalCount`) rather than being fed data directly — every JS file the generic pipeline writes to disk (`generic_downloadFiles.ts`'s external downloads, and `generic_getScriptTags.ts`'s inline-script and data-URI writes) calls `recordJsFileHash(sha256(content))` as a side effect of the write. `generic_crawlPages.ts` instantiates one `StagnationMonitor` per crawl (only if `--stagnation-timein` > 0) and checks it once per BFS loop iteration, alongside the `--max-pages` check — no timers needed, since the loop is already paced by network I/O. See `--stagnation-timein`/`--stagnation-percentage`/`--stagnation-monitor` in `src/index.ts` and the root `CLAUDE.md`'s exit-code table (27) for the cross-flag validation against `--lazyload-timeout`.
