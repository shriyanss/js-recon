...
- **Page visit cap.** `NextJsCrawler` accepts `maxPageVisits` (default `200`; `0` = unlimited). When set, the `pageQueue` loop in `recursivePass` breaks after `totalPagesVisited` reaches the cap and logs a warning. This prevents memory exhaustion on event-heavy sites where every visited page surfaces 10-20 more links. Exposed as `--max-pages` on `lazyload` and `run`.
...
- **`MAX_PAGE_VISITS` cap (200).** `NextJsCrawler` tracks a `totalPagesVisited` counter and stops adding pages to the queue once the cap is reached. This prevents runaway crawl expansion on sites that expose many locale or content-category variants in their navigation — every locale page linking to every other locale can produce thousands of queued visits in a single recursive pass, exhausting the lazyload timeout. 200 page visits is sufficient for virtually all real Next.js apps to converge. Increase only if a legitimate target has more than 200 unique JS-serving routes.
- **Webpack analysis skipped in subsequent-requests mode.** `next_GetLazyResourcesWebpackJs` (a full Puppeteer session that scans JS files for webpack chunk URL builders) runs in `initialDiscovery()` only when `subsequentRequestsFlag = false`. In subsequent-requests passes (steps 3 and 4.5 in `run`), this is a no-op: the bundle hasn't changed and the chunk URL builders were already resolved in the initial lazyload. Running it again wastes 3-6 min per pass.
...
- **HTML scraping pass in `next_SubsequentRequests.ts` is parallelised.** The second pass over the endpoints list (HTML fetch + inline `__next_f.push` extraction) runs with `Promise.all` under the same `max_queue` semaphore as the RSC pass, not sequentially. Touching the concurrency logic here affects both passes.
