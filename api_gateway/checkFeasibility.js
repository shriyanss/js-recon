...
const checkFeasibility = async (url) => {
    console.log(
        chalk.cyan(`[i] Checking feasibility of API Gateway with ${url}`)
    );
    try {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            if (isFireWallBlocking) {
                console.log(
                    chalk.magenta("[!] Please try again without API Gateway")
                );
                return;
            }
        }
        console.log(
            ...
            chalk.dim(
                "However, this doesn't represent the true nature of the firewall used."
            )
        );
    } catch (error) {
        console.log(
